...
class UiDoctorCli {
  static Future<int> run(List<String> arguments) async {
    ...
    if (!targetDir.existsSync()) {
      stderr.writeln('Error: Project directory not found: ${options.projectPath}');
      return 2;
    }
    ...
    if (options.baselinePath != null) {
      ...
      if (!bFile.existsSync()) {
        stderr.writeln('Error: Baseline file not found: ${options.baselinePath}');
        return 2;
      }
      try {
        ...
        stderr.writeln('Error: Failed to parse baseline file: ${options.baselinePath} ($e)');
        return 2;
      }
    }
    ...
    if (options.configPath != null) {
      ...
      try {
        ...
        final valRes = ProjectConfig.parseYaml(cContent, sourcePath: cFile.path);
        if (!valRes.isValid) {
          stderr.writeln('Error: Config validation failed: ${valRes.errors.join(', ')}');
          return 2;
        }
        customConfig = valRes.config;
      } catch (e) {
        stderr.writeln('Error: Failed to load config file: ${options.configPath} ($e)');
        return 2;
      }
    }
  ...
  static _UiCliOptions _parseArgs(List<String> arguments) {
    ...
    for (var index = 0; index < arguments.length; index += 1) {
      ...
      switch (flag) {
        ...
        case '--generate-baseline':
          generateBaselinePath = value ?? '.flutter_dev_intelligence_baseline.json';
          break;
        ...
        case '--color':
          switch (value!.toLowerCase()) {
            ...
            case 'auto':
              ...
              break;
            default:
              stderr.writeln('Error: Invalid --color mode: $value. Choose auto, always, or never.');
              return _UiCliOptions(projectPath: projectPath, errorExitCode: 2);
          }
          break;
    ...
    if (!const {'terminal', 'json', 'markdown'}.contains(format)) {
      stderr.writeln('Error: Unsupported output format: $format. Choose terminal, json, or markdown.');
      return _UiCliOptions(projectPath: projectPath, errorExitCode: 2);
    }

    if (!const {'all', 'ui', 'accessibility', 'performance', 'maintainability', 'assets'}.contains(scope)) {
      stderr.writeln('Error: Unsupported scope: $scope. Choose all, ui, accessibility, performance, maintainability, or assets.');
      return _UiCliOptions(projectPath: projectPath, errorExitCode: 2);
    }
  ...
  static Future<int> _renderAndOutput(
    ...
    if (options.outputPath != null) {
      try {
        ...
        final fileContent = (options.format == 'terminal' && options.colorMode != ColorMode.always)
            ? TerminalReporter.stripAnsi(rendered)
            : rendered;
        await File(options.outputPath!).writeAsString('$fileContent\n');
      } catch (e) {
        stderr.writeln('Error: Failed to write output file: ${options.outputPath} ($e)');
        return 3;
      }
    }
    ...
    if (options.baselinePath != null && options.failOnNew && report.baseline?.status == 'FAILED') {
      return 1;
    }
  ...
  static void _printHelp() {
    stdout.writeln('Flutter Dev Intelligence — UI Doctor (Static UI/UX & Code Health)');
    stdout.writeln('');
    stdout.writeln('Statically inspects Flutter source code and asset configuration to identify');
    stdout.writeln('potential UI, accessibility, maintainability, and performance code smells.');
    stdout.writeln('');
    stdout.writeln('Usage: flutter_dev_intelligence ui-doctor [options]');
    stdout.writeln('');
    stdout.writeln('Options:');
    stdout.writeln('  --project <path>             Target Flutter project directory (default: current directory)');
    stdout.writeln('  --scope <scope>              Analysis scope: all, ui, accessibility, performance, maintainability, assets (default: all)');
    stdout.writeln('  --format <format>            Output format: terminal, json, or markdown (default: terminal)');
    stdout.writeln('  --output <path>              Write report output to specified file path');
    stdout.writeln('  --config <path>              Path to configuration YAML file');
    stdout.writeln('  --baseline[=<path>]          Compare findings against saved baseline snapshot file');
    stdout.writeln('  --generate-baseline[=<path>] Save current findings as a baseline JSON snapshot');
    stdout.writeln('  --fail-on-new / --no-fail-on-new Return exit code 1 if new baseline issues exist (default: true)');
    stdout.writeln('  --color <mode>               Terminal color mode: auto, always, or never (default: auto)');
    stdout.writeln('  --no-color                   Disable terminal color formatting');
    stdout.writeln('  --ascii                      Use ASCII characters for layout formatting');
    stdout.writeln('  --verbose                    Print detailed execution traces');
    ...
    stdout.writeln('  dart run flutter_dev_intelligence ui-doctor --generate-baseline');
    stdout.writeln('  dart run flutter_dev_intelligence ui-doctor --format=json --output=ui-report.json');
  }
}
